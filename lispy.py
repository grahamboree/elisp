...
class Env(dict):
	...
	def add_globals(self):
		...
		self.update({
			...
			'read-char': readchar,
			'read': read,
			'write': lambda x, port = sys.stdout:
				port.write(to_string(x)),
			'display': lambda x, port = sys.stdout:
				port.write(x if isinstance(x,str) else to_string(x))
		})
...
#	return expand(Reader.read(inport), toplevel = True)
#
########################################## Reader
#class Reader(object):
#	@staticmethod
...
#				return [quotes[token], Reader.read(inport)]
#			elif token is eof_object:
#				raise SyntaxError('unexpected EOF in list')
#			else:
#				return Reader._atom(token)
#		# body of read:
#		token1 = inport.next_token()
...
#			return '"%s"' % x.encode('string_escape').replace('"',r'\"')
#		elif isinstance(x, list):
#			return '('+' '.join(map(Writer.to_string, x))+')'
#		elif isinstance(x, complex):
#			return str(x).replace('j', 'i')
#		else:
#			return str(x)
...
#			val = eval(x)
#			if val is not None and out:
#				print >> out, Writer.to_string(val)
#		except Exception as e:
#			print '%s: %s' % (type(e).__name__, e)
#
################# Prelude functions
...
#	"""Signal a syntax error if predicate is false."""
#	if not predicate:
#		raise SyntaxError(Writer.to_string(x) + ': ' + msg)
#
#def expand_quasiquote(x):
#	"""Expand `x => 'x; `,x => x; `(,@x y) => (append x y)"""
#	if not is_pair(x):
...
################# MACROS
#
#macro_table = {_let : Prelude.let} ## More macros can go here
#
## prelude
#eval(parse("""(begin
#	; short circuit-ing 'and'
#	(define-macro and (lambda args
#		(if (null? args) #t
#			(if (= (length args) 1) (car args)
#				`(if ,(car args) (and ,@(cdr args)) #f)))))
